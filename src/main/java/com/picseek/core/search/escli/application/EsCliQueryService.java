...
import com.picseek.core.search.escli.domain.EsCliIndexValidator;
import com.picseek.core.search.escli.interfaces.rest.dto.EsClusterHealthDTO;
import com.picseek.core.search.escli.interfaces.rest.dto.EsClusterStatsDTO;
import com.picseek.core.search.escli.interfaces.rest.dto.EsDocGetDTO;
import com.picseek.core.search.escli.interfaces.rest.dto.EsDocHitDTO;
...
import com.picseek.core.search.escli.interfaces.rest.dto.EsIndexStatsDTO;
...
public class EsCliQueryService {
    ...
    private <T> T monitor(String endpoint, CheckedSupplier<T> supplier) {
        ...
        try {
            ...
            meterRegistry.counter("picseek.escli.requests", "endpoint", endpoint, "outcome", "success").increment();
            return value;
        } catch (BusinessException e) {
            meterRegistry.counter("picseek.escli.requests", "endpoint", endpoint, "outcome", "business_error").increment();
            auditFailure(endpoint, e.getMessage());
            throw e;
        } catch (Exception e) {
            meterRegistry.counter("picseek.escli.requests", "endpoint", endpoint, "outcome", "error").increment();
            BusinessException mapped = mapException(endpoint, e);
            auditFailure(endpoint, mapped.getMessage());
            throw mapped;
        } finally {
            sample.stop(Timer.builder("picseek.escli.latency")
