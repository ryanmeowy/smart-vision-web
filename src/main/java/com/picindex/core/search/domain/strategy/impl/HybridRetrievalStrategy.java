...
import com.picindex.core.search.domain.port.SearchRerankPort.RerankItem;
import com.picindex.core.search.domain.ranking.RrfFusionService;
...
import static com.picindex.core.common.constant.SearchConstant.DEFAULT_RESULT_LIMIT;
import static com.picindex.core.search.domain.util.ScoreUtil.mapScoreToPercentage;
...
public class HybridRetrievalStrategy implements RetrievalStrategy {
    ...
    private List<ImageSearchResultDTO> applyCrossEncoderRerank(String keyword,
        ...
        meterRegistry.counter("picindex.search.rerank.calls").increment();

        List<RerankItem> rerankResults = searchRerankPort.rerank(keyword, docs, docs.size());
        sample.stop(Timer.builder("picindex.search.rerank.latency")
                .description("Cross-encoder rerank latency")
                .register(meterRegistry));
        if (CollectionUtil.isEmpty(rerankResults)) {
            meterRegistry.counter("picindex.search.rerank.fallback", "reason", "empty_result").increment();
            return window.stream().limit(limit).collect(Collectors.toList());
        }
    ...
    private void recordWindowMetrics(int windowSize, int candidateSize) {
        DistributionSummary.builder("picindex.search.rerank.window.size")
                .description("Rerank window size")
                .register(meterRegistry)
                .record(windowSize);

        double ratio = candidateSize <= 0 ? 0d : (double) windowSize / (double) candidateSize;
        DistributionSummary.builder("picindex.search.rerank.window.ratio")
                .description("Rerank window size / candidate size ratio")
                .register(meterRegistry)
                .record(ratio);

        meterRegistry.counter(
                        "picindex.search.rerank.window.hit",
